...
describe("Next.js Compatibility (Integration Tests)", () => {
	...
	beforeEach(async () => {
		...
		testDir = path.join(
			"/tmp",
			`nextjs-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
		);
	...
	for (const structure of projectStructures) {
		it(`should work with ${structure.name} structure`, async () => {
			...
			if (structure.type === "pages") {
				await fs.promises.mkdir(path.join(testDir, "pages"), {
					recursive: true,
				});
				await fs.promises.writeFile(
	...
	it("should work in monorepo structure", async () => {
		...
		await fs.promises.mkdir(path.join(testDir, "apps", "web"), {
			recursive: true,
		});
