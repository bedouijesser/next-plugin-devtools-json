...
for (const pattern of patterns) {
  try {
    const result = execSync(`find /tmp -maxdepth 1 -type d -name "${pattern.replace('/tmp/', '')}" 2>/dev/null`, { encoding: 'utf8' });
    const directories = result.trim().split('\n').filter(dir => dir && dir.length > 0);
    
    if (directories.length > 0) {
      console.log(`📁 Found ${directories.length} directories matching ${pattern}:`);
