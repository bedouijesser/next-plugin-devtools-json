  ...
  return (
    <div>
      ...
      <p>This is a Pages Router project</p>
		...
		await fs.promises.writeFile(
			path.join(testDir, "pages", indexFile),
			indexContent,
		);
	} else {
		// App Router
  ...
  return (
    <html lang="en">
      <body>{children}</body>
		...
		await fs.promises.writeFile(
			path.join(testDir, "app", pageFile),
			pageContent,
		);
		await fs.promises.writeFile(
			path.join(testDir, "app", layoutFile),
			layoutContent,
		);
	}
...
async function testPluginSetup(testDir, config) {
	...
	try {
		...
		// Copy dist directory
		await fs.promises.cp(distPath, path.join(nodeModulesDir, "dist"), {
			recursive: true,
		});
		...
		if (fs.existsSync(binPath)) {
			await fs.promises.cp(binPath, path.join(nodeModulesDir, "bin"), {
				recursive: true,
			});
		}
		...
		const testPackageJson = JSON.parse(
			await fs.promises.readFile(testPackageJsonPath, "utf-8"),
		);

		if (!testPackageJson.devDependencies) {
			testPackageJson.devDependencies = {};
		}
		testPackageJson.devDependencies["next-plugin-devtools-json"] =
			"file:./node_modules/next-plugin-devtools-json";
	...
	if (
		!packageJson.devDependencies ||
		!packageJson.devDependencies["next-plugin-devtools-json"]
	) {
	...
	try {
		if (config.typescript) {
			...
			if (
				!configContent.includes("import") ||
				!configContent.includes("export default")
			) {
				throw new Error(
					"ESM file must contain import and export default statements",
				);
			}
...
async function runTests() {
	...
	if (passed === total) {
		console.log(
			"🎉 All tests passed! The plugin is compatible with all tested configurations.",
		);
	} else {
		console.log(
			"⚠️  Some tests failed. Check the individual test outputs above.",
		);
		process.exit(1);
	}
}
