...
async function testDevToolsEndpoint(port = 3000) {
	...
	for (let i = 0; i < maxRetries; i++) {
		try {
			...
			console.log(
				`⏳ Waiting for dev server... (attempt ${i + 1}/${maxRetries})`,
			);
...
async function createQuickTest() {
	...
	await fs.promises.mkdir(path.join(testDir, "pages", "api"), {
		recursive: true,
	});
	...
	await execAsync(`npm install "${localPluginPath}" --save-dev`, {
		cwd: testDir,
	});

	const setupScript = path.join(
		testDir,
		"node_modules/next-plugin-devtools-json/bin/setup.js",
	);
	...
	devServer.stdout.on("data", (data) => {
		...
		for (const line of lines) {
			if (
				line.includes("DevTools JSON server") ||
...
async function runQuickTest() {
	...
	try {
		...
		if (success) {
			console.log(
				"🎉 Quick test PASSED - DevTools endpoint working, server properly cleaned up, and graceful shutdown!",
			);
		} else {
			console.log(
				"❌ Quick test FAILED - Either DevTools endpoint not working, server not properly cleaned up, or forced shutdown required",
			);
			process.exit(1);
		}
	} catch (error) {
...
async function checkPortsCleanedUp(ports) {
			...
			if (pids.length > 0) {
				...
				try {
					const { stdout: processInfo } = await execAsync(
						`ps -p ${pids.join(",")} -o pid,ppid,command --no-headers`,
						{ stdio: "pipe" },
					);
					...
					stillRunning.push({
						port,
						pids,
						processInfo: "Could not get process info",
						portInfo: "",
					});
				}
			}
	...
	if (stillRunning.length > 0) {
		...
		for (const { port, pids, processInfo, portInfo } of stillRunning) {
